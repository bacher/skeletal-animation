...
void main() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (weight > 0.0) {
      ...
      vec3 v = a_position - bone_pos;
      vec3 offset = v + 2.0 * cross(q.xyz, cross(q.xyz, v) + q.w * v); 
      vec3 final_pos = bone_pos_current + offset;
...
void main() {
  ...
  outColor = vec4(v_intensity, 0, 0, 1);
