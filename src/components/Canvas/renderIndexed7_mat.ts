...
function applyBones(
  ...
  for (const bone of bones) {
    ...
    if (anim) {
      const animMatArray = anim.transforms[animationControl.frameIndex];

      const initMat = mat4.fromValues(...(bone.matrix as Mat4));
      const animMat = mat4.fromValues(...(animMatArray as Mat4));
      // mat4.mul(animMat, animMat, rot);
      // mat4.mul(initMat, initMat, rot);
    ...
    // -- or --
    const jointPos = vec3.transformMat4(vec3.create(), bone.offset, rot);
    // -- or --
    ...
    // work
    const resRot = mat4.mul(mat4.create(), rot, boneQuat);
    // const resRot = mat4.mul(mat4.create(), boneQuat, rot);
    // -- or --
    // const resRot = quat.mul(quat.create(), boneQuat, rot);

    // rotBuffer.set([resRot.x, resRot.y, resRot.z, resRot.w], bone.index * 4);
    const q = mat4.getRotation(quat.create(), resRot);
    rotBuffer.set(Array.from(q), bone.index * 4);

    if (bone.children) {
      applyBones(bone.children, animationParts, buffers, resRot, newPos);
    }
  }
}
