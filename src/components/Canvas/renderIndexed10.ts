...
function applyBones(
  ...
  for (const bone of bones) {
    ...
    if (anim) {
      const transformIndex1 = Math.floor(animationControl.frameIndex);
      const transformIndex2 = Math.ceil(animationControl.frameIndex);

      const transform1 = mat4.fromValues(
        ...(anim.transforms[transformIndex1] as Mat4),
      );

      if (
        transformIndex1 === transformIndex2 ||
        anim.transforms.length <= transformIndex2
      ) {
        boneMat = transform1;
      } else {
        const delta = animationControl.frameIndex - transformIndex1;
        const transform2 = mat4.fromValues(
